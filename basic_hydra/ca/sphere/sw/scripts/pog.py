...
print()
k_in = input(' Choice (default 3)? ')
k = int(k_in or 3)

dname_list=('qq','qq','zz','dd','gg','hh')
field_list=('qq','qa','zz','dd','gg','hh')

dfile=dname_list[k-1]+'.r4'
field=field_list[k-1]
...
# Last time in data:
t1_def='{x:.0f}'.format(x=time[-1])

# Select time to show:
t1_in = input(' Time to show (default '+str(t1_def)+')? ')
t1 = float(t1_in or t1_def)
...
# Output filename:
outfile=field+'_n'+str(ng)+'_t'+str(int(t1))+'_lat'+str(int(rlatc))+'_lon'+str(int(rlonc))+'.eps'
...
if option==3:
   ...
   in_file.close()
   Zb=np.empty([nt,ng+2])
   Zb[0:nt,1:ng+1]=raw_array[frame1*(N+1)+1:(frame1+1)*(N+1)].reshape(nt,ng)
   Z=Z-Zb
...
# Add time:
ax1.text(0.0, 0.945, '$t = {x:.0f}$'.format(x=t1), fontsize=24, transform=ax1.transAxes)

#=========================================================================
# Save image:
fig1.savefig(outfile, format='eps', dpi=300)

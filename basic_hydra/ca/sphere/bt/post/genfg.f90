...
double precision:: rhs(ngv),etdu(ngv),htdu(ngv),rdtu(ngv-1)
...
double precision:: dlf,dlfi,xend,fac,pxu,pxc,pyu,pyc
double precision:: t,aspsqm1,rhou,rsumu,dsumui
double precision:: qdsp,qdnp,avqa,rlonf
...
!----------------------------------------------------------------
 !Open file containing residual vorticity (if a CLAM simulation):
open(40,file='cont/qd.r4',iostat=ivar,form='unformatted', &
...
if (ivar .eq. 0) then
  ...
  close(40) 
   ...
   !Interpolate residual from the half to the full grid:
  ...
  enddo
   ...
   !Initialise area weights for interpolation of residual (qq0)
...
!------------------------------------------------------------------
 !Remove global average vorticity:
avqa=zero
do ix=1,ntv
  do iy=1,ngv-1
...
!------------------------------------------------------------
 !Perform 4th order compact interpolation to get qa on the 
 !half-grid in latitude:
htdu(1)=one/f74
etdu(1)=-f14*htdu(1)
do j=2,ngv-1
...
htdu(ngv)=one/(f74+f14*etdu(ngv-1))
 !The above initialises the procedure
...
integer:: k,ka,i,j,jump,ioff,ncr

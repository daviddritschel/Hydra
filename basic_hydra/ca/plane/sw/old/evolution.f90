...
double precision, allocatable :: dsamp(:)
...
if (nsamp .gt. 0) then
 ...
 !PV fields:
 !Store qd^n & qd^n+0.25*dt*sqd(t^n) for efficiency in the iteration below:
qdi=qd
qdm=qd+dt4*sqd
 !Preliminary guess for qd^{n+1}:
qd=two*dissi*(qdm+dt4*sqd)-qdi
 !Store qs^n+0.5*dt*sqs(t^n) for efficiency in the iteration below:
qsi=qs+dt2*sqs
...
ngs=f12*sgs+hdti*(gsi+nds)
 !Preliminary guess for ds^{n+1} & gs^{n+1}:
sgs=simp*(ngs+f12*(sgs+hdti*sds))
ds=two*sgs-dsi
...
do iter=1,niter
  ...
  enddo
   ...
   !Obtain new guesses for the gridded PV fields (in spectral space):
  qd=two*dissi*(qdm+dt4*sqd)-qdi
  qs=qsi+dt2*sqs

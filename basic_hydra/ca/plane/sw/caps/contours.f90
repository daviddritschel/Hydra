...
double precision,parameter:: glf=twopi/dble(ngf),glfi=oms*dble(ngf)/twopi

 !Contour to grid conversion quantities (ultra-fine grid used in congen):
double precision:: xgu(ngu)
double precision,parameter:: glu=twopi/dble(ngu),glui=oms*dble(ngu)/twopi
...
integer:: ixfw(ngu),ix0w(ngu),ix1w(ngu)
...
integer:: i,ix0,ix1,iy0,iy1
 ...
 !Passed arrays:
double precision:: uu(ng,ng),vv(ng,ng)
...
double precision:: xx,yy,cc,p,sqjump,px0,qavg0
integer:: ixc(nptq),ngc(nptq)
integer:: i,ia,ix,iy,ixdif,ixbeg,jump,ncr,ngh,ngff,mix,mixm1,miy
logical:: crossx(nptq)
...
qavg0=dsumi*sum(qa(1:ng,1:ng))
qc=qa(1:ng,1:ng)-qavg0
...
integer:: lev,levp,levt,nlev,i,ib,ie,is,isb,ibeg,iend,j,nptlev,nseg

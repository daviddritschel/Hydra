...
double precision:: xr(npm),yr(npm)
integer:: indr(nm),npr(nm),i1r(nm),i2r(nm)
integer:: nr,nptr
...
if (nq .gt. 0) then
  ...
  call con2ugrid
   ...
   !Bi-linear interpolate the residual PV in qr (in global commons)
   !to the fine grid and add to qa:
  do ix=1,nxu
    ixf=ixfw(ix)
    ix0=ix0w(ix)
    ix1=ix1w(ix)
    ...
    do iy=1,nyu
      ...
      qa(iy,ix)=qa(iy,ix)+w00(iyf,ixf)*qr(iy0,ix0) &
                       & +w10(iyf,ixf)*qr(iy1,ix0) &
                       & +w01(iyf,ixf)*qr(iy0,ix1) &
                       & +w11(iyf,ixf)*qr(iy1,ix1)
  ...
  enddo
   ...
   !Check if field requires contouring by computing l1 norm of qr:
  if (garea*sum(abs(qr)) .lt. small) return
   !Note: garea is the grid box area, glx*gly

   !No contours: interpolate qr (which here contains the full field - 
   !see initialisation in caps.f90) to the fine grid as qa:
  do ix=1,nxu
    ixf=ixfw(ix)
    ix0=ix0w(ix)
    ix1=ix1w(ix)
    ...
    do iy=1,nyu
      ...
      qa(iy,ix)=w00(iyf,ixf)*qr(iy0,ix0)+w10(iyf,ixf)*qr(iy1,ix0) &
             & +w01(iyf,ixf)*qr(iy0,ix1)+w11(iyf,ixf)*qr(iy1,ix1)
...
do icr=1,ncr
  if (free(icr)) then
     !A new contour (indexed nr) starts here:
    nr=nr+1
    indr(nr)=levt
    ibeg=nptr+1
    i1r(nr)=ibeg
    ...
    keep=.false.
    do
      call renode(xd,yd,npd,xr(ibeg),yr(ibeg),npr(nr))
       !Delete contour if deemed too small (see renode_closed):
      if (npr(nr) .eq. 0) exit
      call renode(xr(ibeg),yr(ibeg),npr(nr),xd,yd,npd)
       !Delete contour if deemed too small (see renode_closed):
      if (npd .eq. 0) exit
      call renode(xd,yd,npd,xr(ibeg),yr(ibeg),npr(nr))
       !Delete contour if deemed too small (see renode_closed):

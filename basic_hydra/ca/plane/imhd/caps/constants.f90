...
double precision,parameter:: gp1=0.21132486541d0,gp2=one-gp1

 !Domain half-lengths, inverses and limits:
...
double precision,parameter:: garea=glx*gly,dsumi=one/dble(nx*ny)
 ...
 !Limit maximum time step to half the grid data save time:
double precision,parameter:: dtmax=f12*tgsave

 !Logical control variables:
logical,parameter:: beffect=(beta .ne. zero),barotropic=(kd .eq. zero)
logical,parameter:: heating=(rtherm .gt. zero),friction=(rekman .gt. zero)
logical,parameter:: zforcing=(esrz .gt. zero),aforcing=(esra .gt. zero)

...
#======================================================================
#   Contouring routine for data produced by sw caps codes
...
def make_plot(Z):
    ...
    zmax_def=abs(Z).max()
    zmax_in=input('Maximum magnitude of field to show? (default '+str(zmax_def)+') ')
    zmax=float(zmax_in or zmax_def)
    zmin=-zmax

    dZ_def=contint(zmin,zmax)
...
#    ax1.set_ylabel('$y$', fontsize=25)
    ...
    # Draw contours:
    im1=ax1.contour(Z,clevels,colors='k',linewidths=2.0,extent=(-np.pi,np.pi,-np.pi,np.pi),origin='lower')
    # Add labels:
#    plt.clabel(im1, clevels, inline=1, fmt='%4.2f', fontsize=14)
    plt.clabel(im1, clevels, inline=1, fontsize=14)

    #------------------------------------------------------------------
    # Save figure after cropping:
    plt.savefig(filename+'_con.eps', format='eps', dpi=600)
    print()
    print( ' *** To view the image, type')
    print()
    print( ' gv '+filename+'_con.eps')
    print()
    
if __name__ == "__main__":
    #==========================================================================
    # Select field to display:
    field_in=input('Field to image (dd, gg, hh, qq or zz)? (default zz) ')
    field=str(field_in or 'zz')
    ...
    kfr_in=input('Time frame to image? (default 100) ')
    kfr=int(kfr_in or 100)
    if kfr < 10:
       frame='000'+str(kfr)
    elif kfr < 100:
       frame='00'+str(kfr)

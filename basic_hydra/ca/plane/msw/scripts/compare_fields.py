...
dir_list=['ng256kd4eps0.100gamma2.0bzrat0.0/','ng512kd4eps0.100gamma2.0bzrat0.0/','ng1024kd4eps0.100gamma2.0bzrat0.0/']
ndir=len(dir_list)

# Corresponding grid resolutions:
ng_list=np.array([256,512,1024])
...
lab_list=['${\\mathrm{Rm}}=200$','${\\mathrm{Rm}}=800$','${\\mathrm{Rm}}=3200$']
...
for k,direc in enumerate(dir_list):
   ...
   # Select frames and plot:
   for m in range(3):
      j=int((t[m]+0.0001)/dt)
      d[0:ng,0:ng]=raw_array[j*(N+1)+1:(j+1)*(N+1)].reshape(ng,ng).T
      if field == 'bz':
         # Remove average:
         davg=np.mean(d[:,:])
         print(davg)
         d[0:ng,0:ng]=d[0:ng,0:ng]-davg
      # Add periodic edges:
      d[ng,0:ng]=d[0,0:ng]
      ...
      # Plot image:
      im=ax0.imshow(d,cmap=cm.seismic,vmin=-dmax,vmax=dmax,extent=(-np.pi,np.pi,-np.pi,np.pi),origin='lower',interpolation='bilinear')

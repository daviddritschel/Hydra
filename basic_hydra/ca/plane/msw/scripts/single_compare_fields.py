...
print()
t_in=input(' Time to show (default 25)? ')
t=float(t_in or 25.0)
...
if option==1:
   ...
   in_file=open(direc[0]+'b'+acron+'.r8','r')
   raw_array=np.fromfile(in_file,dtype=np.float64)
   in_file.close()
   Z1[0:ng,0:ng]=raw_array[frame*(N+1)+1:(frame+1)*(N+1)].reshape(ng,ng)

   in_file=open(direc[1]+'b'+acron+'.r8','r')
   raw_array=np.fromfile(in_file,dtype=np.float64)
   in_file.close()
   Z2[0:ng,0:ng]=raw_array[frame*(N+1)+1:(frame+1)*(N+1)].reshape(ng,ng)

   in_file=open(direc[2]+'b'+acron+'.r8','r')
   raw_array=np.fromfile(in_file,dtype=np.float64)
   in_file.close()
   Z3[0:ng,0:ng]=raw_array[frame*(N+1)+1:(frame+1)*(N+1)].reshape(ng,ng)

   in_file=open(direc[3]+'b'+acron+'.r8','r')
   raw_array=np.fromfile(in_file,dtype=np.float64)
   in_file.close()
   Z4[0:ng,0:ng]=raw_array[frame*(N+1)+1:(frame+1)*(N+1)].reshape(ng,ng)
   ...
   in_file.close()
   in_file=open(direc[0]+'b'+acron+'.r8','r')
   raw_arrayb=np.fromfile(in_file,dtype=np.float64)
   in_file.close()
   Z1[0:ng,0:ng]=raw_array[frame*(N+1)+1:(frame+1)*(N+1)].reshape(ng,ng)-raw_arrayb[frame*(N+1)+1:(frame+1)*(N+1)].reshape(ng,ng)
   ...
   raw_arrayb=np.fromfile(in_file,dtype=np.float64)
   in_file.close()
   Z2[0:ng,0:ng]=raw_array[frame*(N+1)+1:(frame+1)*(N+1)].reshape(ng,ng)-raw_arrayb[frame*(N+1)+1:(frame+1)*(N+1)].reshape(ng,ng)
   ...
   raw_arrayb=np.fromfile(in_file,dtype=np.float64)
   in_file.close()
   Z3[0:ng,0:ng]=raw_array[frame*(N+1)+1:(frame+1)*(N+1)].reshape(ng,ng)-raw_arrayb[frame*(N+1)+1:(frame+1)*(N+1)].reshape(ng,ng)
   ...
   raw_arrayb=np.fromfile(in_file,dtype=np.float64)
   in_file.close()
   Z4[0:ng,0:ng]=raw_array[frame*(N+1)+1:(frame+1)*(N+1)].reshape(ng,ng)-raw_arrayb[frame*(N+1)+1:(frame+1)*(N+1)].reshape(ng,ng)
...
print()
zmin1_in=input('Minimum value to show in   SW   simulation? (default '+str(zmin1)+') ')
zmin1=float(zmin1_in or zmin1)
zmax1_in=input('Maximum value to show in   SW   simulation? (default '+str(zmax1)+') ')
zmax1=float(zmax1_in or zmax1)

print()
zmin2_in=input('Minimum value to show in SW-bal simulation? (default '+str(zmin2)+') ')
zmin2=float(zmin2_in or zmin2)
zmax2_in=input('Maximum value to show in SW-bal simulation? (default '+str(zmax2)+') ')
zmax2=float(zmax2_in or zmax2)

print()
zmin3_in=input('Minimum value to show in   GN   simulation? (default '+str(zmin3)+') ')
zmin3=float(zmin3_in or zmin3)
zmax3_in=input('Maximum value to show in   GN   simulation? (default '+str(zmax3)+') ')
zmax3=float(zmax3_in or zmax3)

print()
zmin4_in=input('Minimum value to show in GN-bal simulation? (default '+str(zmin4)+') ')
zmin4=float(zmin4_in or zmin4)
zmax4_in=input('Maximum value to show in GN-bal simulation? (default '+str(zmax4)+') ')
zmax4=float(zmax4_in or zmax4)

...
if (nq .gt. 0) then
  ...
  if (beffect) then
    !Subtract beta*y before interpolation:
    do ix=1,nx
      do iy=1,ny
        qq(iy,ix)=qq(iy,ix)-beta*(ymin+gly*dble(iy-1))
      enddo
    enddo
  endif

   !Interpolate qq to the ultra-fine grid as qa:
  do ix=1,nxu
    ixf=ixfw(ix)
    ix0=ix0w(ix)
    ix1=ix1w(ix)
  ...
  if (beffect) then
    !Add back beta*y before contouring:
    do ix=1,nxu
      do iy=1,nyu
        qa(iy,ix)=qa(iy,ix)+beta*ygu(iy)
      enddo
    enddo
  endif

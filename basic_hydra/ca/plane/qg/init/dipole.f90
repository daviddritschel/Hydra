...
double precision:: xg(nxu),yg(nyu)
...
glxf=ellx/dble(nxu)
do ix=1,nxu
  xg(ix)=xmin+glxf*dble(ix-1)
enddo
glyf=elly/dble(nyu)
do iy=1,nyu
  yg(iy)=ymin+glyf*dble(iy-1)
enddo

do ix=1,nxu
  do iy=1,nyu
    r=sqrt(xg(ix)**2+yg(iy)**2)+1.d-15
    if (r .ge. one) then
      f=ae*bessk1(p*r)/r
    else
...
do ix=1,nx
  do iy=1,ny
    qq(iy,ix)=qa(iy,ix)+beta*(gly*dble(iy-1)+ymin)
  enddo
enddo

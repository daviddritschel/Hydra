...
!  Here, we use the SW tendencies for delta and gamma, and set the
!  non-hydrostatic pressure to zero.

!  This can be used both for initialisation and for post-processing.

!  Adapted from dgbal.f90 25 March 2020 by D G Dritschel @ St Andrews
!######################################################################
...
! Local variables:
double precision:: htot(ng,ng),wka(ng,ng)

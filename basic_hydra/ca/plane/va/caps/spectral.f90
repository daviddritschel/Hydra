...
double precision:: c2g2(ng,ng),simp(ng,ng),filt(ng,ng)
double precision:: diss(ng,ng),pdis(ng,ng),rdis(ng,ng)
  ...
  do kx=1,ng
    ...
    if (rks .gt. rkfsq) then
      ...
      helm(kx,ky)=one/opak(kx,ky)
       !P*G operator product:
      pgop(kx,ky)=pope(kx,ky)*opak(kx,ky)
       !(grad^2 - 3/H^2)^{-1}:
    ...
    simp(kx,ky)=one/(pope(kx,ky)*(dt2i+diss(kx,ky))**2-opak(kx,ky))
     !Re-define damping operator for use in q_l evolution:
    diss(kx,ky)=two/(one+dt2*diss(kx,ky))
     !Redefine P by its inverse:
    pope(kx,ky)=one/pope(kx,ky)
  enddo
enddo
...
double precision:: htot(ng,ng),wkp(ng,ng),cc(ng,ng),dd(ng,ng)
...
call yderiv(ng,ng,hrky,wkc,vds)
 ...
 !Obtain a physical space copy of qs, ds & gs:
wka=qs
...
do while (enorm .gt. tole)
  ...
  qbar=dsumi*sum(qq)
   ...
   !Invert [c^2*grad^2-f*(f+<q>)]h = 
   !                f*(q+h*(q-<q>)-(H^2/3)*(1+h)*J(delta,h)) - gamma_l
  wkp=cof*(qq+hh*(qq-qbar)-htot*cc)
  call ptospc(ng,ng,wkp,wkb,xfactors,yfactors,xtrig,ytrig)

  fqbar=cof*qbar
  wka=filt*(wkb-gs)/(opak-fqbar)
  ...
  htot=one+hh
   ...
   !Re-compute C = (H^2/3)*J(h,delta):
  call jacob(hh,dd,cc)
  cc=hbsq3*cc
  call dealias(cc)

   !Compute relative vorticity zz = (1+hh)*(f+qq-cc) - f:
  wkp=qq*htot
  qadd=-dsumi*sum(wkp)
  ...
  wkd=vds+wkd  !vds is the fixed divergent part of vv
   ...
   !Convert to physical space as (wka,wkc):
  call spctop(ng,ng,wkb,wka,xfactors,yfactors,xtrig,ytrig)
  call spctop(ng,ng,wkd,wkc,xfactors,yfactors,xtrig,ytrig)

   !Add mean flow (uio,vio):
  uio=-sum(hh*wka)*dsumi
  vio=-sum(hh*wkc)*dsumi
  wka=wka+uio
  wkc=wkc+vio

   !Compute rms error in uu & vv:
  durms=sum((uu-wka)**2+(vv-wkc)**2)

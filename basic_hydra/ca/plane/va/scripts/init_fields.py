...
# This script plots the initial PV, delta and gamma fields in the
# current directory.
...
in_file=open('qq_init.r8','r')
raw_array=np.fromfile(in_file,dtype=np.float64)
in_file.close()
Z1[0:ng,0:ng]=raw_array[1:N+1].reshape(ng,ng).T

Z2=np.empty([ng+1,ng+1])
in_file=open('dd_init.r8','r')
raw_array=np.fromfile(in_file,dtype=np.float64)
in_file.close()
Z2[0:ng,0:ng]=raw_array[1:N+1].reshape(ng,ng).T

Z3=np.empty([ng+1,ng+1])
in_file=open('gg_init.r8','r')
raw_array=np.fromfile(in_file,dtype=np.float64)
in_file.close()
Z3[0:ng,0:ng]=raw_array[1:N+1].reshape(ng,ng).T
...
print()
print('    Field      Min field value      Max field value')
print(' -----------   ---------------      ---------------')
print('      q        ',"{:14.10f}".format(zmin1),'     ',"{:14.10f}".format(zmax1))
print('    delta      ',"{:14.10f}".format(zmin2),'     ',"{:14.10f}".format(zmax2))
print('    gamma      ',"{:14.10f}".format(zmin3),'     ',"{:14.10f}".format(zmax3))
...
ax1.set_title('$q$', fontsize=30)
ax2.set_title('$\\delta$', fontsize=30)
ax3.set_title('$\\gamma$', fontsize=30)

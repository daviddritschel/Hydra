...
! Special version of the spectral module which allows for bottom topography in the domain.
! This topography is read in from the init file topo.dat.
use constants
...
double precision:: wk1d(0:nx+ny),wk2d(0:ny,0:nx)
...
double precision:: dtopo(0:nx)
double precision:: ftopo(0:ny,0:nx),gtopo(0:ny,0:nx),htopo(0:ny,0:nx)
...
double precision:: ytopo(0:nx),slope(0:nx),dslope(0:nx)
...
!-------------------------------------------------------
 !Read in bottom topography (it is wise to centre this on ymin):
open(11,file='topo.dat',status='old')
do ix=0,nx
  read(11,*) ytopo(ix)
enddo
close(11)
...
do ix=0,nx
  dtopo(ix)=one/(ymax-ytopo(ix))
enddo

 !Carry out an x cosine transform on ytopo:
call c06hbf(1,nxp1,ytopo,'s',cosxtab,wk1d,ifail)

 !Take derivatives spectrally by wavenumber multiplication:
dslope(0)=zero
do kx=1,nx
  slope(kx)=-rkxf(kx)*ytopo(kx)
  dslope(kx)=rkxf(kx)*slope(kx)
enddo

 !Invert transforms:
call c06haf(1,nx,slope(1),'s',sinxtab,wk1d,ifail)  
call c06hbf(1,nxp1,dslope,'s',cosxtab,wk1d,ifail)
slope(0)=zero
slope(nx)=zero

do ix=0,nx
  do iy=0,ny
    yyc=elly-gly*dble(iy)
    dyydy=elly*dtopo(ix)
    dyydx=-slope(ix)*yyc*dtopo(ix)
    ftopo(iy,ix)=one-dyydx**2-dyydy**2
    gtopo(iy,ix)=-two*dyydx
    htopo(iy,ix)=-yyc*dtopo(ix)*(dslope(ix)+two*slope(ix)**2*dtopo(ix))
  enddo
enddo
...
double precision:: zz(0:ny,0:nx),uu(0:ny,0:nx),vv(0:ny,0:nx)
 !Local parameters & arrays:
double precision,parameter:: toler=1.d-9
double precision:: ss(0:nx,0:ny),pp(0:ny,0:nx),zzrhs(0:ny,0:nx)
double precision:: pbot(nx),ptop(nx),cppy(nym1,nx)
double precision:: plft(ny),prgt(ny),cppx(nxm1,ny)
double precision:: ppold(ny,nx),pps(nx,ny)
double precision:: ppy(nx,0:ny),dpdy(0:ny,nx)
double precision:: ppxy(0:nx,0:ny),d2pdxdy(0:ny,0:nx)
double precision:: ppyy(nx,ny),d2pdy2(ny,nx)
...
do while (psierr .gt. tol) 

 !Build right hand side for inversion:
if (niter .gt. 0) then
   !Transform ppold to spectral space:
  call ptospc_ss(ppold,pps)

   !Compute d(ppold)/dy = ppy spectrally: 
  call yderiv_ss(pps,ppy)

   !Compute d2(ppold)/dxdy = ppxy spectrally:
  call xderiv_sc(ppy,ppxy)

   !Compute d2(ppold)/dy2 = ppyy spectrally:
  call yderiv_sc(ppy,ppyy)

   !Transform ppy back to physical space as dpdy:
  call spctop_sc(ppy,dpdy)

   !Transform ppxy back to physical space as d2pdxdy:
  call spctop_cc(ppxy,d2pdxdy)

   !Transform ppy back to physical space as dpdy:
  call spctop_ss(ppyy,d2pdy2)

  do ix=1,nxm1
    do iy=1,nym1
      zzrhs(iy,ix)=zz(iy,ix)+ftopo(iy,ix)*d2pdy2(iy,ix) &
                          & +gtopo(iy,ix)*d2pdxdy(iy,ix) &
                          & +htopo(iy,ix)*dpdy(iy,ix)
    enddo
  enddo  
  do ix=1,nxm1
    zzrhs(0,ix) =zz(0,ix)+gtopo(0,ix)*d2pdxdy(0,ix)+htopo(0,ix)*dpdy(0,ix)
    zzrhs(ny,ix)=zz(ny,ix)
  enddo
  do iy=0,ny
    zzrhs(iy,0) =zz(iy, 0)+gtopo(iy, 0)*d2pdxdy(iy,0)
    zzrhs(iy,nx)=zz(iy,nx)+gtopo(iy,nx)*d2pdxdy(iy,nx)
  enddo
endif

 !Solve for psi (pp):

 !(1) compute mean zzrhs (zbar):
zbar=zero
do ix=1,nxm1
  zbar=zbar+zzrhs(0,ix)+zzrhs(ny,ix)
enddo
do iy=1,nym1
  zbar=zbar+zzrhs(iy,0)+zzrhs(iy,nx)
enddo
zbar=f12*zbar+f14*(zzrhs(0,0)+zzrhs(ny,0)+zzrhs(0,nx)+zzrhs(ny,nx))
do ix=1,nxm1
  do iy=1,nym1
    zbar=zbar+zzrhs(iy,ix)
  enddo
enddo
zbar=zbar/dble(nx*ny)

 !(2) Remove mean vorticity from zzrhs:
do ix=0,nx
  do iy=0,ny
    pp(iy,ix)=zzrhs(iy,ix)-zbar
  enddo
enddo
 
 !(3) FFT zzrhs and invert to get uncorrected streamfunction pp:
...
!Compute error in pp field:
psierr=zero
do ix=1,nxm1
  do iy=1,nym1
    psierr=max(psierr,abs(pp(iy,ix)-ppold(iy,ix)))
    ppold(iy,ix)=pp(iy,ix)
  enddo
enddo

niter=niter+1
enddo !Close main do-while loop 

...
implicit integer(i-n)
 ...
 !Local parameter:
double precision,parameter:: fracgl=0.25d0
!fracgl: Fraction of a grid length that we move edge grid points inwards 
!        to ensure edge values get populated during trajectory intepolation 
...
do ix=1,nxm1
  xig(ix)=dble(ix)
enddo
xig(nx)=dble(nx)-fracgl

yig(0)=fracgl
do iy=1,nym1
  yig(iy)=dble(iy)
enddo
yig(ny)=dble(ny)-fracgl
 
!-----------------------------------------------------------------
 !Read in initial vorticity (zs) and buoyancy (bb):
open(11,file='zz_init.dat',status='old')
read(11,*) t
open(12,file='bb_init.dat',status='old')
read(12,*) t
do ix=0,nx
  do iy=0,ny
    read(11,*) zs(iy,ix)
...
!--------------------------------------
 !Open all diagnostic and output files:
     
if (t .lt. small) then 
   !This is the start of a new run
  open(20,file='monitor.dat',status='unknown')
  open(30,file='norms.dat',status='unknown')
  open(40,file='complexity.dat',status='unknown')
  open(50,file='zz.dat',status='unknown')
  open(60,file='bb.dat',status='unknown')
  open(70,file='ene.dat',status='unknown')
  open(80,file='zspec.dat',status='unknown')

else
   !This is a continuation of a previous run
  open(20,file='monitor.dat',status='old',access='append')
  open(30,file='norms.dat',status='old',access='append')
  open(40,file='complexity.dat',status='old',access='append')
  open(50,file='zz.dat',status='old',access='append')
  open(60,file='bb.dat',status='old',access='append')
  open(70,file='ene.dat',status='old',access='append')
  open(80,file='zspec.dat',status='old',access='append')

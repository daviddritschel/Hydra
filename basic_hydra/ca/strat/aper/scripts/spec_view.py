...
# Set the number of frames:
  nframes = int(len(raw_data)/(3*kmax+5))  

# Shape the data array into a useful shape for plotting:
  frames=range(0,nframes)
  tim_eles = [i*(3*kmax+5)+j for i in frames for j in range(0,5)]
  shaped_data = np.delete(raw_data,tim_eles)[0:(3*kmax+5)*nframes].reshape((nframes,kmax,3))
  global ic 
  ...
  im = ax.plot(shaped_data[ic].transpose()[0],shaped_data[ic].transpose()[1],'r-')
  im2 = ax.plot(shaped_data[ic].transpose()[0],shaped_data[ic].transpose()[2],'b-')
  ...
  def on_press(event):
    ...
    if event.key=='=':
      ...
      ylim=axes.get_ylim()
      axes.clear()
      axes.plot(shaped_data[ic].transpose()[0],shaped_data[ic].transpose()[1],'r-')
      axes.plot(shaped_data[ic].transpose()[0],shaped_data[ic].transpose()[2],'b-')

...
!-----------------------------------------------------------------
 !Read in initial vorticity (zs) and buoyancy (bb) and topography (h):
...
call init_spectral(bbmax-bbmin,hh)
...
domarea=zero
do ix=0,nxm1
  domarea=domarea+confac(0,ix)+confac(ny,ix)
enddo
domarea=f12*domarea

do ix=0,nxm1
  do iy=1,nym1
    domarea=domarea+confac(iy,ix)
  enddo
enddo

domarea=garea*domarea
...
write(*,'(a,i8,a,i9,2(a,f9.5))') '  nb = ',nb,'  nptb = ',nptb, &
                                 '  db = ',bjump,'  <b> = ',bavg
...
write(*,'(a,i8,a,i9,2(a,f9.5))') '  nz = ',nz,'  nptz = ',nptz, &
                                 '  dz = ',zjump,'  <z> = ',zavg

...
! casl suite of f90 codes in a period channel
...
integer:: nextb(npm),nptb,nb
 ...
 !and quantities needed for potential energy calculation (in this module):
double precision:: xgf(0:nxfm1),ygf(0:nyf)
...
integer:: ixfp(0:nxfm1),iyfp(0:nyf)
...
integer:: ibx(0:nxu)
 ...
 !Fine-grid area weightings used in vorticity tendency calculations:
...
iyp(ny)=ny
 ...
 !Used for interpolation of vorticity source (getzzsrc):
do ix=0,nxfm2
  ixfp(ix)=ix+1
enddo
ixfp(nxfm1)=0
...
wdzdt=bjump/gareaf
 ...
 !Fine x-grid lines needed for contour-to-grid conversion (con2grid) and
 !fine y-grid lines (relative to ymin) needed for potential energy (getpe):
do ix=0,nxfm1
...
subroutine getzzsrc(bxsrc,t0,dyoridx,dyoridy)
...
double precision:: bxsrc(0:ny,0:nxm1),bysrc(0:ny,0:nxm1)
double precision:: bxsrcf(0:nyfp1,0:nxfm1),bysrcf(0:nyfp1,0:nxfm1)

!--------------------------------------------------------------------
 !Initialise sources to zero everywhere:
do ix=0,nxfm1
  do iy=0,nyf
    bxsrcf(iy,ix)=zero
    bysrcf(iy,ix)=zero
...
do j=1,nb
  ...
  do i=is,ie
    ...
    do k=1,ndiv
      ...
      avgbxsrc=wdzdt*(y1-y2)
      avgbysrc=wdzdt*(x2-x1)
      ...
      bxsrcf(iy0,ix0)=bxsrcf(iy0,ix0)+pyc*pxc*avgbxsrc
      bxsrcf(iy0,ix1)=bxsrcf(iy0,ix1)+pyc*px*avgbxsrc
      bxsrcf(iy1,ix0)=bxsrcf(iy1,ix0)+py*pxc*avgbxsrc
      bxsrcf(iy1,ix1)=bxsrcf(iy1,ix1)+py*px*avgbxsrc

      bysrcf(iy0,ix0)=bysrcf(iy0,ix0)+pyc*pxc*avgbysrc
      bysrcf(iy0,ix1)=bysrcf(iy0,ix1)+pyc*px*avgbysrc
      bysrcf(iy1,ix0)=bysrcf(iy1,ix0)+py*pxc*avgbysrc
      bysrcf(iy1,ix1)=bysrcf(iy1,ix1)+py*px*avgbysrc
...
do ix=0,nxfm1
  bxsrcf(0,  ix)=two*bxsrcf(0,  ix)
  bxsrcf(nyf,ix)=two*bxsrcf(nyf,ix)  
  bysrcf(0,  ix)=two*bysrcf(0,  ix)
  bysrcf(nyf,ix)=two*bysrcf(nyf,ix)  
enddo

 !Average to inversion grid by repeated 1-2-1 averages in each direction:
call coarsen(bxsrcf,bxsrc)
call coarsen(bysrcf,bysrc)

 !Combine and scale the sources:
do ix=0,nxm1
  do iy=0,ny
    bxsrc(iy,ix)=bxsrc(iy,ix)*dyoridy(iy,ix)-bysrc(iy,ix)*dyoridx(iy,ix)
  enddo
enddo
...
subroutine con2grid(qq,xq,yq,dq,nextq,nptq)
...
! The gridded field is returned in the array qq, but domain average 
! is restored afterwards using the routine restore.
...
qbot(0)=zero
 !Corner value cannot be determined a priori; qavg is restored in restore() after
...
implicit integer(i-n)
 ...
 !Passed arrays:
double precision:: qa(0:nyfp1,0:nxfm1),qq(0:ny,0:nxm1)

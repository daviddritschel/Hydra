...
double precision,parameter:: dyyui=dble(nyu)/(two*hly)
...
! nodes (xq(i),yq(i)), i = 1, ..., nptq.  Across each contour 
! the PV jumps by dq = qjump(iz) in layer iz.  This routine
! ensures that the average PV equals qavg(iz) upon return.

! The gridded PV field is returned in the array qc.
...
do iz=1,nz
  if (jl2q(iz) .gt. 0) then
    ...
    call average(qc(0,0,iz),qavg0)

    qadd=qavg(iz)-qavg0
    do ix=0,nxm1
      do iy=0,ny
        qc(iy,ix,iz)=qc(iy,ix,iz)+qadd
      enddo
    enddo
...
enddo
 ...
 !Copy nodes back to their original arrays:
do i=1,nptq
  xq(i)=xa(i)
  yq(i)=ya(i)
  nextq(i)=nexta(i)
enddo
 ...
 !Copy contour indices back to their original arrays:
do j=1,nq
  i1q(j)=i1a(j)
  i2q(j)=i2a(j)
  indq(j)=inda(j)
